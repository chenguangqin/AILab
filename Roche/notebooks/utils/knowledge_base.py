...
class BedrockKnowledgeBase:
    ...
    def create_bedrock_execution_role_multi_ds(self, bucket_names=None, secrets_arns=None):
        ...
        # create bedrock execution role（幂等：已存在则复用，失败重跑不再报 EntityAlreadyExists）
        try:
            ...
            print(f"Role {self.kb_execution_role_name} already exists - reusing it.")
            bedrock_kb_execution_role = self.iam_client.get_role(RoleName=self.kb_execution_role_name)

        # create and attach the policies to the bedrock execution role（幂等）
        for policy_name, policy_document, description in policies:
            ...
            # attach_role_policy 幂等：重复附加同一策略不会报错
            self.iam_client.attach_role_policy(
